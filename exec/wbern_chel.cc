...
//g++ `root-config --cflags --evelibs` -std=c++17 -O3 -Wall -Wextra -Wpedantic -Werror -Wno-float-equal -Wno-sign-compare -I ./src -o ./exec/wbern_chel ./src/EFTFitter.cc ./exec/wbern_chel.cc
...
int main(int argc, char** argv) {
  ...
  //const std::string inDir = "/nfs/dust/cms/user/afiqaize/cms/rand/eftRivet_290118/EFTFitter/wbern_0519/root/", opName = argv[1];
    const std::string inDir = "/afs/desy.de/user/z/zimermma/work/EFTFitter/wbern_0314/root/", opName = argv[1];
...
//const std::string covMatrix ="TotalStatCovMatrix_AllVarNorm_rebinnedA";
  ...
  const std::string outDir = "/afs/desy.de/user/z/zimermma/work/EFTFitter/output-2016-1D/sigleObsFit/" + covMatrix + "/" + opName + "/";
  ...
  std::vector<int> v_fit_var;
    ...
    //eft.draw1DChi2({ {opName, {opName, {/* op range in min, max */}, {0., 9.999}, {-opRange + eps, opRange - eps} }} }, outDir, v_sample);
      eft.draw1DChi2({ {opName, {"#hat#mu_{t}", {/* op range in min, max */}, {0., 9.999}, {-0.02 + eps, +0.05 - eps} }} }, outDir, v_sample);
    ...
    for (int iRes = 0; iRes < v_current_result.size(); ++iRes) {
      if (v_current_result.at(iRes).at(0) == nullptr) continue;
        ...
        v_current_result.at(iRes).at(0)->SetName( (sample + "_" + toStr(v_hStr.at(iRes)) + "_sigma1").c_str() );
        v_current_result.at(iRes).at(0)->Write( (sample + "_" + toStr(v_hStr.at(iRes)) + "_sigma1").c_str() );

        v_current_result.at(iRes).at(1)->SetName( (sample + "_" + toStr(v_hStr.at(iRes)) + "_sigma2").c_str() );
        v_current_result.at(iRes).at(1)->Write( (sample + "_" + toStr(v_hStr.at(iRes)) + "_sigma2").c_str() );
